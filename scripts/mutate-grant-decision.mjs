...
const MUTANTS = [
  ["M1 decide: always grant",
    ...
    "  return CONTEXT_ISOLATION_CAPABILITY;"],
  ...
  ["M11 streamToolProof: allow same-event use+result",
    ...
    "    void sawUseInThisEvent; void sawResultInThisEvent;"],
  ["M12 structured proofs: skip skill proof",
    CANARY, '  if (!skillPair) return isoFail("skill_proof_missing", wantSkill);', "  if (!skillPair) { /* mutant */ }"],
  ["M13 structured proofs: skip plugin proof",
    CANARY, '  if (!pluginPair) return isoFail("plugin_proof_missing", PLUGIN_CANARY_SKILL);', "  if (!pluginPair) { /* mutant */ }"],
  ["M14 structured proofs: skip framework proof",
    CANARY, '  if (!frameworkPair) return isoFail("framework_proof_missing", wantFramework);', "  if (!frameworkPair) { /* mutant */ }"],
  ["M15 structured proofs: skip mcp proof",
    CANARY, '  if (!mcpPair) return isoFail("mcp_proof_missing", wantMcp);', "  if (!mcpPair) { /* mutant */ }"],
  ...
  ["M17 structured proofs: drop init skill/plugin requirement",
    ...
    "  void wantSkill; void wantPlugin;"],
  ["M18 structured proofs: drop verifyInitSurfaceExclusion",
    ...
    "  void verifyInitSurfaceExclusion;"],
...
const misses = results.filter((r) => r.status === "PATTERN-MISS");
if (misses.length) {
  console.error(
    `\nFATAL: ${misses.length} mutant(s) PATTERN-MISS — battery stale, not a pass: ` +
    misses.map((r) => r.name).join(", ")
  );
  process.exit(1);
}
