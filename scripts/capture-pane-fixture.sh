...
CAPTURE_FILES_EXPORT="$(printf '%s\n' "${CAPTURE_FILES[@]}")"
CAPTURE_TIMESTAMPS_EXPORT="$(printf '%s\n' "${CAPTURE_TIMESTAMPS[@]}")"
export CAPTURE_FILES_EXPORT CAPTURE_TIMESTAMPS_EXPORT INPUT_TEXT INPUT_AT FIXTURE_ROOT
...
const captureFiles = process.env.CAPTURE_FILES_EXPORT?.split("\n").filter(Boolean) ?? [];
const captureTimestamps = process.env.CAPTURE_TIMESTAMPS_EXPORT?.split("\n").filter(Boolean) ?? [];
