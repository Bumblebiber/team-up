...
/**
 * Import a capability from Git at an exact revision. The ref is resolved to a
 * commit before import and recorded, so a moving branch or retagged release
 * never mutates an installed pool entry. No checked-in script is executed.
 */
export function importGitCapability(
  { url, ref = "HEAD" },
  ...
  try {
    ...
    const commit = String(
      exec("git", ["rev-parse", "HEAD"], { cwd: repo, encoding: "utf8" })
    ).trim();
